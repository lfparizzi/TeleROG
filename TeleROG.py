...
Selecione uma opção:
    ...
    /ticket <Observação>   - Abra um ticket para um técnico de nível 2

    /relatorio <Observação> - Descreva um relatório da atual situação do cliente
"""


####################################################################################################
####################################### FilePaths ##################################################
filename = 'c:\\Users\\syrtain\\Desktop\\telerog.txt'
...
@bot.message_handler(commands=["ticket", "Ticket", "TICKET", "TicKet", "tiket"])
def nome(mensagem):
  
  if mensagem.text.lower() == "/ticket": #condição impede que o bot quebre ao se digitar o comando sem argumentos
    print(getUser(mensagem)+"digitou o /ticket sem texto e foi orientado")
    resposta = "Insira o texto após o comando, exemplo: \n/ticket Problemas de Nateamento no cliente"
  else:
    bot.reply_to(mensagem, "Ticket aberto para o suporte de nível 2")


    if soHospedagem == "Windows": #Verifica se o bot está em um windows
      ticketID = str(random.randrange(99999))
      print(getUser(mensagem)+"abriu o ticket "+ticketID)
      resposta = "Ticket ID: "+ticketID
    elif soHospedagem == "Linux":
      print(getUser(mensagem)+"abriu um ticket")
    else:
      print("System - erro a definir Sistema Operacional de hospedagem na busca de NOME, verificar parâmetros globais")
      resposta = "Erro a definir Sistema Operacional de hospedagem, contate o administrador para verificar parâmetros globais do TeleROG.py"
  bot.reply_to(mensagem, resposta) #ato de responder


@bot.message_handler(commands=["relatorio", "Relatorio", "Relatório", "relatório", "relat", "RELATORIO", "RELATÓRIO"])
def nome(mensagem):
  
  if mensagem.text.lower() == "/relatorio": #condição impede que o bot quebre ao se digitar o comando sem argumentos
    print(getUser(mensagem)+"digitou o /relatorio sem texto e foi orientado")
    resposta = "Insira o texto após o comando, exemplo: \n/relatorio cliente possui o modem da empresa e utiliza Wi-Fi próprio"
  else:
    bot.reply_to(mensagem, "Relatorio concluído")


    if soHospedagem == "Windows": #Verifica se o bot está em um windows
      relatID = str(random.randrange(99999))
      print(getUser(mensagem)+"abriu o relatório "+relatID)
      resposta = "Relatório ID: "+relatID
    elif soHospedagem == "Linux":
      print(getUser(mensagem)+"abriu o relatório"+relatID)
    else:
      print("System - erro a definir Sistema Operacional de hospedagem na busca de NOME, verificar parâmetros globais")
